...
export async function getUniversalAuth(request) {
  try {
    ...
    const authResult = clerkGetAuth(request);
    ...
    if (authHeader?.startsWith('Bearer ')) {
      ...
      try {
        ...
        const sessionToken = await backend.sessions.verifySession(token, token);
 ...
 */
/**
 * Compatible getAuth function for existing API routes
 * @param {Request} request - The incoming request object
...
export async function getAuth(request) {
  ...
  if (userId && !error) {
    return { success: true, userId, message: 'Authenticated successfully' };
  }
...
export async function getSellerAuth(request) {
  ...
  try {
    ...
    const isSeller = Boolean(sellerResult) === true;
    ...
    console.error('Seller authorization error:', authError);
    return { 
      userId, 
      isSeller: false, 
