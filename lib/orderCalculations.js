 ...
 */
export const calculateOrderTotal = (cartItems, deliveryAddress, promoCode = '', forceDhaka = null, dynamicOffer = null) => {
  try {
    // Calculate subtotal
    ...
    // Calculate discount
    const discountResult = calculateDiscount(subtotalResult.subtotal, promoCode, dynamicOffer);
...
export const convertCartToOrderItems = (cartItems, products) => {
  ...
  for (const [itemKey, quantity] of Object.entries(cartItems)) {
    ...
    // Get variant-specific image with improved color matching
    let productImage = product.images?.[0] || '';
    
    // Try multiple approaches to find the correct color image
    if (product.colorImages && product.colorImages.length > 0 && color && color !== 'default') {
      // First try exact match (case-sensitive)
      let colorImage = product.colorImages.find(ci => ci.color === color);
      
      // If no exact match, try case-insensitive match
      if (!colorImage) {
        colorImage = product.colorImages.find(ci => 
          ci.color && ci.color.toLowerCase() === color.toLowerCase()
        );
      }
      
      // If still no match, try partial match (in case of variations like "Dark Blue" vs "blue")
      if (!colorImage) {
        colorImage = product.colorImages.find(ci => 
          ci.color && (ci.color.toLowerCase().includes(color.toLowerCase()) || 
          color.toLowerCase().includes(ci.color.toLowerCase()))
        );
      }
      
      // Use the found image or keep the default
      ...
      if (!colorImage) {
        console.log(`⚠️ No color image found for color: "${color}" in product: ${product.name}`);
        console.log(`Available colors: ${product.colorImages.map(ci => ci.color).join(', ')}`);
      }
    }
