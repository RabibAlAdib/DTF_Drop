...
const EditOffer = ({ params }) => {
  ...
  const fetchOffer = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setValidTo(new Date(offer.validTo).toISOString().split('T')[0]);
        setOfferCode(offer.offerCode || '');
        setMinimumOrderValue(offer.minimumOrderValue?.toString() || '');
        setUsageLimit(offer.usageLimit?.toString() || '');
        setOfferType(offer.offerType);
        setPriority(offer.priority?.toString() || '0');
        setCategory(offer.category);
        setShowCountdown(offer.showCountdown);
        setBackgroundColor(offer.backgroundColor || '#FF6B6B');
        setTextColor(offer.textColor || '#FFFFFF');
        setIsActive(offer.isActive);
        setCurrentImageUrl(offer.offerImage || '');
        setApplicableProducts(offer.applicableProducts?.map(p => p._id || p) || []);
      } else {
        toast.error(response.data.message || "Failed to fetch offer");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const token = await getToken();
      const response = await axios.put(`/api/offer/${id}`, formData, {
        headers: {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Edit Offer</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          ...
          <div className="space-y-4">
            ...
            {currentImageUrl && !removeImage && !offerImage && (
                ...
                {offerImage ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-gray-400">
                    <Image 
                      ...
                      className="mb-2"
                    />
                    <span>Click to upload {removeImage ? 'new' : ''} offer image</span>
            ...
            {/* Cancel Remove Image */}
            {removeImage && currentImageUrl && !offerImage && (
              <button
                type="button"
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            {isUploading ? 'Updating...' : 'Update Offer'}
          </button>
        </div>
      </form>
    </div>
